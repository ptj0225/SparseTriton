...
@triton.autotune(
    configs=[
        ...
        # Larger configs for better occupancy
        triton.Config({'BLOCK_SIZE_N': 256, 'BLOCK_SIZE_C_OUT': 32, 'BLOCK_SIZE_C_IN': 32}, num_warps=8, num_stages=3),
        triton.Config({'BLOCK_SIZE_N': 256, 'BLOCK_SIZE_C_OUT': 64, 'BLOCK_SIZE_C_IN': 32}, num_warps=8, num_stages=3),
        triton.Config({'BLOCK_SIZE_N': 256, 'BLOCK_SIZE_C_OUT': 64, 'BLOCK_SIZE_C_IN': 64}, num_warps=8, num_stages=4),
        triton.Config({'BLOCK_SIZE_N': 128, 'BLOCK_SIZE_C_OUT': 128, 'BLOCK_SIZE_C_IN': 32}, num_warps=8, num_stages=3),
        triton.Config({'BLOCK_SIZE_N': 128, 'BLOCK_SIZE_C_OUT': 128, 'BLOCK_SIZE_C_IN': 64}, num_warps=8, num_stages=4),
    ],
    key=['C_in', 'C_out'],
)
@triton.jit
def implicit_gemm_hash_on_fly_fwd_kernel(
    ...
    for k in range(K_VOL):
        ...
        valid_mask = (in_indices >= 0) & mask_n & (curr_x >= 0) & (curr_y >= 0) & (curr_z >= 0) & (curr_x < spatial_shape_x) & (curr_y < spatial_shape_y) & (curr_z < spatial_shape_z)
...
def implicit_gemm_bwd_feat_kernel(
    ...
    b = tl.load(coords_ptr + off_n * 4, mask=mask_n, other=0)  # (BLOCK_SIZE_N,)
    x = tl.load(coords_ptr + off_n * 4 + 1, mask=mask_n, other=0)  # (BLOCK_SIZE_N,)
    ...
    for k in range(K_VOL):
        ...
        # Keep pointer math in-bounds for masked-out lanes on ROCm.
        in_indices_safe = tl.where(in_indices >= 0, in_indices, 0)
        target_ptrs = d_features_ptr + in_indices_safe[:, None] * C_in + off_cin[None, :]  # (BLOCK_SIZE_N, BLOCK_SIZE_C_IN)
...
def implicit_gemm_bwd_weight_kernel(
    ...
    f_tile = tl.load(
        features_ptr + in_indices_safe[:, None] * C_in + off_cin[None, :],
        mask=valid_mask[:, None] & mask_cin[None, :],
        other=0.0
    )  # (BLOCK_SIZE_N, BLOCK_SIZE_C_IN)
...
class ConvHashOnTheFlyImplicitGEMM(torch.autograd.Function):
    ...
    def forward(ctx, features, weights, coords, kernel_offsets, hash_table_keys, hash_table_vals, spatial_shape, lookup_stride):
        ...
        Args:
            ...
            coords: (N_out, 4) 출력 좌표
            kernel_offsets: (K, 3) 커널 오프셋
            hash_table_keys: 해시 테이블 키
            hash_table_vals: 해시 테이블 값
            spatial_shape: (3,) 입력 공간 크기 (X, Y, Z)
        """
        # 입력 검증 및 차원 확인
